...
function createBuffer (
  ...
  programAttributeInfo: { loc: number , type: string, size: number }, 
  data: number[], 
  opts = defaultBufferOpts
) {
  const { loc, size }  = programAttributeInfo; 
  const { stride, offset, normalized, type, usage } = {...defaultBufferOpts, type: gl.FLOAT, usage: gl.STATIC_DRAW,  ...opts };
  ...
  function createVAO(gl, program, geometry) {
    ...
    program.attributes.forEach((attr) => {
        const data = geometry[attr.name];
      
        const buffer = createBuffer(gl, attr, data );
        buffers[attr.name] = buffer;
...
function getError(gl: WebGL2RenderingContext): string {
...
function debug(gl: WebGL2RenderingContext): void {
  const e = getError(gl);
  if (e) console.error("WebGL Error: ", e);
}
...
export default {
  ...
  calculateNormals(vertices: number[], ind) {
    const
      x = 0,
      y = 1,
      z = 2,
    ...
    for (let i = 0; i < ind.length; i += 3) {
      ...
      // p2 - p1
      v1[x] = vertices[3 * ind[i + 2] + x] - vertices[3 * ind[i + 1] + x];
      v1[y] = vertices[3 * ind[i + 2] + y] - vertices[3 * ind[i + 1] + y];
      v1[z] = vertices[3 * ind[i + 2] + z] - vertices[3 * ind[i + 1] + z];

      // p0 - p1
      v2[x] = vertices[3 * ind[i] + x] - vertices[3 * ind[i + 1] + x];
      v2[y] = vertices[3 * ind[i] + y] - vertices[3 * ind[i + 1] + y];
      v2[z] = vertices[3 * ind[i] + z] - vertices[3 * ind[i + 1] + z];

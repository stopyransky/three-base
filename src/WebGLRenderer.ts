...
import { AttributesCache } from "./core/cache/AttributesCache";
import { ProgramsCache } from "./core/cache/ProgramCache";
import { UniformsCache } from "./core/cache/UniformsCache";
import { BindingStatesCache } from "./core/cache/BindingStateCache";
import { GeometriesCache } from "./core/cache/GeometriesCache";
import { ObjectsCache } from "./core/cache/ObjectsCache";
import { PropertiesCache } from "./core/cache/PropertiesCache";
import { RenderListsCache } from "./core/cache/RenderListsCache";
...
// import { WebGLMorphtargets } from "./core/WebGLMorphtargets";
// import { WebGLShadowMap } from "./core/WebGLShadowMap";
...
function WebGLRenderer(parameters) {
  ...
  let extensions, capabilities, state, info, textures, cubemaps;
  ...
  let materials, renderLists, renderStates, clipping, shadowMap;
  ...
  function initGLContext() {
    ...
    attributesCache = AttributesCache(_gl);
    bindingStatesCache = BindingStatesCache(_gl, attributesCache);
    geometriesCache = GeometriesCache(
      _gl,
      attributesCache,
      bindingStatesCache,
      info
    );
    objectsCache = ObjectsCache(_gl, geometriesCache, attributesCache, info);
    ...
    renderLists = RenderListsCache(propertiesCache);
    ...
    indexedBufferRenderer = new WebGLIndexedBufferRenderer(_gl, info);

    info.programs = programsCache.programs;

    _this.capabilities = capabilities;
    _this.extensions = extensions;
    _this.properties = propertiesCache;
  ...
  this.dispose = function () {
    ...
    propertiesCache.dispose();
    cubemaps.dispose();
    objectsCache.dispose();
    bindingStatesCache.dispose();
  ...
  function deallocateMaterial(material) {
    ...
    propertiesCache.remove(material);
  }

  function releaseMaterialProgramReferences(material) {
    const programs = propertiesCache.get(material).programs;

    if (programs !== undefined) {
      programs.forEach(function (program) {
        programsCache.releaseProgram(program);
      });
    }
  }
  ...
  this.renderBufferImmediate = function (object, program) {
    bindingStatesCache.initAttributes();

    const buffers = propertiesCache.get(object);
    ...
    if (object.hasPositions) {
      ...
      bindingStatesCache.enableAttribute(programAttributes.position);
    ...
    if (object.hasNormals) {
      ...
      bindingStatesCache.enableAttribute(programAttributes.normal);
      _gl.vertexAttribPointer(
    ...
    if (object.hasUvs) {
      ...
      bindingStatesCache.enableAttribute(programAttributes.uv);
    ...
    if (object.hasColors) {
      ...
      bindingStatesCache.enableAttribute(programAttributes.color);
      _gl.vertexAttribPointer(
    ...
    bindingStatesCache.disableUnusedAttributes();
  ...
  this.renderBufferDirect = function (
    ...
    if (material.wireframe === true) {
      index = geometriesCache.getWireframeAttribute(geometry);
      rangeFactor = 2;
    }
    ...
    bindingStatesCache.setup(object, material, program, geometry, index);

    let attribute;
    let renderer = bufferRenderer;

    if (index !== null) {
      attribute = attributesCache.get(index);
    ...
    if (visible) {
      if (object.isGroup) {
        ...
        if (!object.frustumCulled || _frustum.intersectsSprite(object)) {
          ...
          const geometry = objectsCache.update(object);
          const material = object.material;
  ...
  function getProgram(material, scene, object) {
    ...
    if (program !== undefined) {
      ...
      parameters.uniforms = programsCache.getUniforms(material);

      material.onBuild(parameters, _this);

      material.onBeforeCompile(parameters, _this);

      program = programsCache.acquireProgram(parameters, programCacheKey);
    ...
    const uniformsList = UniformsCache.seqWithValue(progUniforms.seq, uniforms);
  ...
  this.setRenderTarget = function (
    ...
    if (
      renderTarget &&
      propertiesCache.get(renderTarget).__webglFramebuffer === undefined
    ) {
    ...
    if (renderTarget) {
      ...
      const __webglFramebuffer = propertiesCache.get(renderTarget)
        .__webglFramebuffer;
      ...
      if (renderTarget.isWebGLCubeRenderTarget) {
        ...
        framebuffer = propertiesCache.get(renderTarget)
    ...
    if (isCube) {
      const textureProperties = propertiesCache.get(renderTarget.texture);
  ...
  this.readRenderTargetPixels = function (
    ...
    let framebuffer = propertiesCache.get(renderTarget).__webglFramebuffer;
    ...
    if (framebuffer) {
      ...
      try {
        ...
        const framebuffer =
          _currentRenderTarget !== null
            ? propertiesCache.get(_currentRenderTarget).__webglFramebuffer
            : null;
        state.bindFramebuffer(_gl.FRAMEBUFFER, framebuffer);
      }
    }
